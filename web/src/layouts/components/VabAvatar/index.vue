...
import { recordRoute, baseURL } from "@/config"; // Import baseURL
import {
  ArrowDown,
  User,
  Setting,
...
// 计算属性
const avatarPath = computed(() => store.getters["user/avatar"]);
const avatar = computed(() => {
  const name = avatarPath.value
  if (!name) return '' // 或者返回默认头像
  // 如果已经是完整URL（虽然不太可能，但为了健壮性）
...
const roleNames = computed(() => store.getters["user/roleNames"]);
const displayRole = computed(() => {
    return (roleNames.value && roleNames.value.length > 0) ? roleNames.value.join(', ') : '未分配'
})
