  ...
  <div class="aliyun-container">
    ...
    <el-card style="margin-top: 20px;" class="table-card">
      <el-table
        ...
        :key="tableKey"
      >
        <el-table-column prop="instance_name" label="实例名称" min-width="200" sortable />
    ...
    >
      <el-form ref="vmForm" :model="editedItem" :rules="formRules" label-width="120px">
        <el-form-item label="实例名称" prop="instance_name">
          <el-input v-model="editedItem.instance_name" placeholder="请输入RDS实例名称" />
        </el-form-item>
        ...
        <el-form-item label="说明" prop="description">
          <el-input v-model="editedItem.description" type="textarea" :rows="3" placeholder="请输入RDS实例说明" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    >
      <div style="margin-bottom: 20px;">
        ...
        <span style="vertical-align: middle; margin-left: 10px;">
          确定要删除RDS实例 <strong>{{ deleteItemData.instance_name }}</strong> 吗？
        </span>
      </div>
...
import {
  listProjectAliyunRDS,
  createProjectAliyunRDS,
  updateProjectAliyunRDS,
  deleteProjectAliyunRDS
} from '@/api/aliyun/rds'
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await listProjectAliyunRDS(params)
...
const openDialog = (item) => {
  editedItem.value = item ? { ...item } : {
    id: null, instance_name: '', project: '', environment: 'prod', owner: '', description: ''
  }
  ...
  vmForm.value.validate(async (valid) => {
    ...
    try {
      if (editedItem.value.id) {
        await updateProjectAliyunRDS(editedItem.value.id, editedItem.value)
        ElMessage.success('更新成功')
      } else {
        await createProjectAliyunRDS(editedItem.value)
...
const confirmDelete = async () => {
  try {
    await deleteProjectAliyunRDS(deleteItemData.value.id)
