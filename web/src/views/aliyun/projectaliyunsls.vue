<template>
  <div class="aliyun-container">
    ...
    <div class="search-bar">
      <el-input
        v-model="searchQuery"
        placeholder="请输入Project名称、项目或负责人"
        clearable
    ...
    <el-card style="margin-top: 20px;" class="table-card">
      <el-table
        ...
        :key="tableKey"
      >
        <el-table-column prop="project_name" label="Project名称" min-width="200" sortable />
    ...
    >
      <el-form ref="vmForm" :model="editedItem" :rules="formRules" label-width="120px">
        <el-form-item label="Project名称" prop="project_name">
          <el-input v-model="editedItem.project_name" placeholder="请输入SLS Project名称" />
        </el-form-item>
        ...
        <el-form-item label="说明" prop="description">
          <el-input v-model="editedItem.description" type="textarea" :rows="3" placeholder="请输入SLS Project说明" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    >
      <div style="margin-bottom: 20px;">
        ...
        <span style="vertical-align: middle; margin-left: 10px;">
          确定要删除SLS Project <strong>{{ deleteItemData.project_name }}</strong> 吗？
        </span>
      </div>
...
import {
  listProjectAliyunSLS,
  createProjectAliyunSLS,
  updateProjectAliyunSLS,
  deleteProjectAliyunSLS
} from '@/api/aliyun/sls'
...
const formRules = {
  project_name: [
    { required: true, message: 'Project名称不能为空', trigger: 'blur' },
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await listProjectAliyunSLS(params)
...
const openDialog = (item) => {
  editedItem.value = item ? { ...item } : {
    id: null, project_name: '', project: '', environment: 'prod', owner: '', description: ''
  }
  ...
  vmForm.value.validate(async (valid) => {
    ...
    try {
      if (editedItem.value.id) {
        await updateProjectAliyunSLS(editedItem.value.id, editedItem.value)
        ElMessage.success('更新成功')
      } else {
        await createProjectAliyunSLS(editedItem.value)
...
const confirmDelete = async () => {
  try {
    await deleteProjectAliyunSLS(deleteItemData.value.id)
