  ...
  <div class="aliyun-container">
    ...
    <el-card style="margin-top: 20px;" class="table-card">
      <el-table
        ...
        :key="tableKey"
      >
        <el-table-column prop="load_balancer_name" label="负载均衡器名称" min-width="200" sortable />
    ...
    >
      <el-form ref="vmForm" :model="editedItem" :rules="formRules" label-width="120px">
        <el-form-item label="负载均衡器名称" prop="load_balancer_name">
          <el-input v-model="editedItem.load_balancer_name" placeholder="请输入SLB负载均衡器名称" />
        </el-form-item>
        ...
        <el-form-item label="说明" prop="description">
          <el-input v-model="editedItem.description" type="textarea" :rows="3" placeholder="请输入SLB实例说明" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    >
      <div style="margin-bottom: 20px;">
        ...
        <span style="vertical-align: middle; margin-left: 10px;">
          确定要删除SLB负载均衡器 <strong>{{ deleteItemData.load_balancer_name }}</strong> 吗？
        </span>
      </div>
...
import {
  listProjectAliyunSLB,
  createProjectAliyunSLB,
  updateProjectAliyunSLB,
  deleteProjectAliyunSLB
} from '@/api/aliyun/slb'
...
const formRules = {
  load_balancer_name: [
    { required: true, message: '负载均衡器名称不能为空', trigger: 'blur' },
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await listProjectAliyunSLB(params)
...
const openDialog = (item) => {
  editedItem.value = item ? { ...item } : {
    id: null, load_balancer_name: '', project: '', environment: 'prod', owner: '', description: ''
  }
  ...
  vmForm.value.validate(async (valid) => {
    ...
    try {
      if (editedItem.value.id) {
        await updateProjectAliyunSLB(editedItem.value.id, editedItem.value)
        ElMessage.success('更新成功')
      } else {
        await createProjectAliyunSLB(editedItem.value)
...
const confirmDelete = async () => {
  try {
    await deleteProjectAliyunSLB(deleteItemData.value.id)
