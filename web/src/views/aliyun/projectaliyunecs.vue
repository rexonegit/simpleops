...
import { Search, Plus, DataAnalysis, Setting, Edit, Delete, Warning } from '@element-plus/icons-vue' // ✅ 引入图标
import {
  listProjectAliyunecs,
  createProjectAliyunecs,
  updateProjectAliyunecs,
  deleteProjectAliyunecs
} from '@/api/aliyun/ecs'
...
const fetchData = async () => {
  ...
  try {
    ...
    // 获取响应
    const res = await listProjectAliyunecs(params)
  ...
  vmForm.value.validate(async (valid) => {
    ...
    try {
      if (editedItem.value.id) {
        await updateProjectAliyunecs(editedItem.value.id, editedItem.value)
        ElMessage.success('更新成功')
      } else {
        await createProjectAliyunecs(editedItem.value)
...
const confirmDelete = async () => {
  try {
    await deleteProjectAliyunecs(deleteItemData.value.id)
