  ...
  <div class="vmware-container">
    ...
    <el-card style="margin-top: 20px;" class="table-card">
      ...
      >
        ...
        <el-table-column prop="created_at" label="创建时间" min-width="180" sortable :formatter="formatDate" />
        <el-table-column prop="updated_at" label="更新时间" min-width="180" sortable :formatter="formatDate" />
...
import {
  ...
  updateProjectVMware,
  deleteProjectVMware
} from '@/api/datacenter/projectvmware'
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await listProjectVMware(params)
...
const fetchStats = async () => {
  try {
    const res = await listProjectVMware({ pageSize: 'all', search: searchQuery.value })
    const payload = res.data || res
    const allData = Array.isArray(payload) ? payload : (payload.results || payload.list || [])
    stats.value = {
  ...
  vmForm.value.validate(async (valid) => {
    ...
    try {
      if (editedItem.value.id) {
        await updateProjectVMware(editedItem.value.id, editedItem.value)
        ElMessage.success('更新成功')
      } else {
        await createProjectVMware(editedItem.value)
...
const confirmDelete = async () => {
  try {
    await deleteProjectVMware(deleteItemData.value.id)
...
<style scoped>
.vmware-container { padding: 20px; background-color: #f5f7fa; }
