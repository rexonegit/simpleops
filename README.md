...
| 序号 | 资源类型             | 模块路径               | 说明                            |
| :--: | -------------------- | ---------------------- | ------------------------------- |
|  1   | ECS 云服务器         | `aliyun/ecs`           | 实例详情、规格、网络、归属项目  |
|  2   | OSS 对象存储         | `aliyun/oss`           | Bucket 列表、地域、存储量、归属 |
|  3   | RAM 访问控制         | `aliyun/ram`           | 用户、角色、权限策略展示        |
|  4   | 弹性公网 IP          | `aliyun/eip`           | EIP 详情、绑定资源、带宽        |
|  5   | 公网 NAT 网关        | `aliyun/snat`          | SNAT/DNAT 规则、关联 EIP        |
|  6   | RDS 云数据库         | `aliyun/rds`           | 实例信息、连接地址、归属项目    |
|  7   | 安全组               | `aliyun/securitygroup` | 规则明细、关联主机信息(修改功能未测)             |
|  8   | Web 应用防火墙 (WAF) | `aliyun/waf`           | WAF基础信息              |
|  9   | SLB 负载均衡         | `aliyun/slb`           | 监听、后端服务器、VIP           |
|  10  | NAS 文件存储         | `aliyun/nas`           | 文件系统、挂载点、容量          |
|  11  | SLS 日志服务         | `aliyun/sls`           | Project、Logstore、索引         |
|  12  | DNS 解析             | `aliyun/dns`           | 域名、解析记录                  |

**核心能力：**

- 🔄 资源同步后自动匹配项目信息并展示资源归属明细
- 🏷️ 每条资源均可追溯所属项目、环境类型（生产/测试/开发）、负责人
- 📋 统一展示面板，支持按项目、环境、资源类型筛选
